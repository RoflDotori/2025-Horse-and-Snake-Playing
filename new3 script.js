...
async function shareToKakao() {
    try {
        ...
        const resultTitle = resultContainer.querySelector('h3')?.textContent || '2025 도토리숲의 뱀과 말';
        const resultDescription = resultContainer.querySelector('p')?.textContent || '나의 2025년 운세';
        ...
        const isKakaoInstalled = isMobile && /kakao/i.test(userAgent);
        
        if (isKakaoInstalled) {
            // 카카오톡이 설치된 모바일 환경
            if (!Kakao.isInitialized()) {
                Kakao.init('7e8fa75a7bf66918d29ad100dced7d7d');
            }
            ...
            await Kakao.Share.sendDefault({
                ...
                content: {
                    ...
                    imageUrl: 'https://dotori-forest-make-2025puns.netlify.app/images/snake-wave.png',
                    link: {
            ...
            if (navigator.share) {
                try {
                    ...
                    console.log('Web Share API 공유 실패:', error);
                }
            }
            
            // Web Share API를 사용할 수 없는 경우 URL 복사로 대체
